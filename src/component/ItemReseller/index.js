...
import {change_to_qty_reseller, selected_reseller} from '../../redux';
...
const ItemReseller = ({
  ...
  const quantity = (type) => {
    if (type === 'min') {
      ...
      if (qty <= 1) {
        setIsDisabled(true);
        setQty(0);
        dispatch(change_to_qty_reseller(qty, cart.id, parseInt(products.price), type));
        // console.log('qty', qty)
      }
    } else if (type === 'plush') {
      setQty(qty + 1);
      setIsDisabled(false);
      dispatch(change_to_qty_reseller(qty + 1, cart.id, parseInt(products.price), type));
    }
    // console.log(type)
  };
  ...
  // const checkItem =() => {
  //   // setIsSelected()
  //   dispatch(selected_reseller(cart.id, isSelected))
