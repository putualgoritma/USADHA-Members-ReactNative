...
import {change_to_qty_amaintain, delete_amaintain, selected_amaintain} from '../../redux';
...
const ItemAmaintain = ({
  ...
  const quantity = (type) => {
    if (type === 'min') {
      ...
      if (qty <= 1) {
        setIsDisabled(true);
        setQty(0);
        dispatch(change_to_qty_amaintain(qty, cart.id, parseInt(products.price), type));
        // console.log('qty', qty)
      }
    } else if (type === 'plush') {
      setQty(qty + 1);
      setIsDisabled(false);
      dispatch(change_to_qty_amaintain(qty + 1, cart.id, parseInt(products.price), type));
    }
    // console.log(type)
  };
  ...
  // const checkItem =() => {
  //   // setIsSelected()
  //   dispatch(selected_amaintain(cart.id, isSelected))
