...
const Profile = ({ navigation }) => {
  ...
  const [pointReseller, setPointReseller] = useState(0)  
  ...
  const getHU = () => {
    ...
    Axios.get(Config.API_LIST_HU_MEMBER+ `?owner_id=${userReducer.owner_id}&exc=${userReducer.id}`, 
      ...
      console.log('kkkkd',result.data.data)
  ...
  const getPoint = () => {
    Axios.get(Config.API_POINT + `${userReducer.id}`, {
      ...
      .then((result) => {
        console.log('data balance_reseller_points', result.data.data[0].balance_reseller_points)
        ...
        setPointReseller(parseInt(result.data.data[0].balance_reseller_points))
        // getAgen()
      }).catch((e) => {
        alert('koneksi error, mohon buka ulang aplikasinya')
        console.log(e);
  ...
  if (loading || loading1 || !form.phone) {
    return (
      <Releoder />
    )
  }
      ...
      <View style={styles.container}>
        ...
        <View style={{width : '100%',
         ...
         {products.length > 0 && products.map((product)=> {          
          return(
            <TouchableOpacity style={{width : 160, backgroundColor : '#ffffff', padding : 5, marginBottom : 5,}} onPress ={() => switchUser({id: product.id})} key={product.id}>
              <View style={{ padding : 5, borderRadius : 3, borderWidth : 0.1, borderColor : colors.disable, backgroundColor:'#f9fcfb'}}>
                <View style={{flex:1}}>
 ...
 />
            ...
            <View style={{ maxWidth: '100%', marginBottom: 20, flexDirection: 'row' }}>
              <Text style={{ flex: 2 }}>Poin Belanja Reseller :</Text>
              <Text style={{ flex: 4, fontWeight: 'bold' }}>{Rupiah(pointReseller)}</Text>
            </View>
