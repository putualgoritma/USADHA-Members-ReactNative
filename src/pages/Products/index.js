...
import { add_to_package,add_to_amaintain,add_to_reseller } from './../../redux';
...
const Products = ({ navigation, route }) => {
  ...
  const amaintainReducer = useSelector((state) => state.AmaintainReducer.item);
  ...
  // var penanda = false;
  const resellerReducer = useSelector((state) => state.ResellerReducer.item);
  ...
  const apiProducts = () => {
    ...
    else if(dataType=='Reseller'){
      apiList=Config.API_LIST_PRODUCT_RESELLER;
    }
  ...
  const insertCart = (product) => {
    ...
    if(dataType=='Reseller'){
      ...
      let penanda = false;
      resellerReducer.some(function (entry, i) {
      ...
      if (!penanda) {
        ...
        navigation.navigate('Reseller', { dataForm: dataForm, dataType: dataType });
    ...
    else if(dataType=='Amaintain'){
    // mencari data jika ada yang sama di keranjang
    let penanda = false;
    amaintainReducer.some(function (entry, i) {
      if (entry.id == product.id) {
        penanda = true;
        return true;
      }
    ...
    if (!penanda) {
      dispatch(add_to_amaintain(item, 1));
      navigation.navigate('AutoMaintain', { dataForm: dataForm, dataType: dataType });
      Alert.alert('Add to Cart');
    } else {
      Alert.alert('item sudah ada di dalam keranjang');
    }
