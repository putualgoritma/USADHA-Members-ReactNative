...
const Keranjang = ({navigation}) => {
  ...
  useEffect(() => {
    ...
    navigation.navigate('PackageToken', {dataForm: null, dataType : 'Checkout', activationType : null})
    }
    if(isFocused){
        ...
        {cartState.item.length == 0? 
          ...
          ( 
            <ButtonCustom
              ...
              width ='40%'
              color= {colors.btn}
              func ={() => { navigation.navigate('PackageToken', {dataForm: null, dataType : 'Checkout', activationType : null})}}
            />
          )
        }
